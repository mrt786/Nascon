    ...
    const fetchReport = async () => {
      try {
        ...
        const { data } = await axios.get(
          'http://localhost:3000/admin/reports/participant-demographics',
          { headers: { Authorization: `${token}` } }
        );
        setReport(data);
      } catch (err) {
        console.error(err);
        setError(err.response?.data?.error || 'Failed to load demographics report.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-slate-800 min-h-screen text-white pt-20">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {report.length === 0 ? (
          <p className="text-center text-gray-400">No demographics data available.</p>
        ) : (
...
export default ParticipantDemographicsReport;
