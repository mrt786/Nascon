...
const AdminAuth = () => {
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    navigate(tab === 'login' ? '/admin-login' : '/admin-signup');
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-[350px] text-center">
        <h2 className="text-2xl font-bold mb-4 text-slate-800">{activeTab === 'login' ? 'Admin Login Form' : 'Admin Signup Form'}</h2>
        <AuthToggle activeTab={activeTab} setActiveTab={setActiveTab} tabname={"admin"} />
