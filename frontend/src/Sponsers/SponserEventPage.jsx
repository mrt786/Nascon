// File: src/pages/SponserEventPage.jsx
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map(ev => (
            <div key={`${ev.event_id}-${ev.sponsor_level}"`} className="bg-slate-900 text-white rounded-xl shadow p-6">
              ...
              <p className="text-sm mb-1">Category: {ev.category}</p>
              <p className="text-sm mb-1">Level: {ev.sponsor_level}</p>
              <p className="text-sm mb-4">Amount: Rs. {ev.sponsor_amount}</p>
              <LoginButton
                text="Select Package"
                btype="button"
                onClick={() => handleSelect(ev.event_id, ev.sponsor_level)}
              />
            </div>
          ))}
        </div>
...
export default SponserEventPage;
